...
exports.user = function(cb) {
    ...
    var url = LASTFM_API_URL + '?method=user.getinfo&user=' + process.env.LASTFM_USERNAME + '&api_key=' + process.env.LASTFM_API_KEY + '&format=json';
...
exports.topActivity = function(cb) {
    ...
    async.series([_fetchTopArtists, _fetchTopAlbums, _fetchTopTracks], function(err, results) {
        if (!err) {
            var topActivity = {
                artists: results[0],
                albums: results[1],
